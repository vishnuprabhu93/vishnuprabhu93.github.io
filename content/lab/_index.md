---
title: "XR BioSim Lab"
mission: "At the XR BioSim Lab (Extended Reality, Biosensing, and Simulation Lab), we study how extended reality, multimodal biosensing, computational modeling, and AI driven adaptive systems can be combined to improve training, human performance, and well-being across complex, high-stakes domains. Our work integrates immersive VR/AR environments with physiological signal streams (e.g., EEG, fNIRS, EDA, HRV, pupillometry) using mathematical modeling, simulation, and machine learning to build digital twins that sense, predict, and respond to human state in real time. Much of our applied work focuses on clinical training and healthcare workforce challenges, but the underlying methods extend to any domain where humans must learn, adapt, and perform under pressure."
currentStudents:
  - name: "Sayed Mohammad (Dariush) Sajjadi"
    role: "Doctoral student · 2025–present"
    photo: "/images/students/dariush-sajjadi.jpg"
    brief: "Dariush is a Ph.D. student and Graduate Research Assistant at the University of Central Florida, specializing in Modeling and Simulation. He holds a Bachelor's degree in Mechanical Engineering with a focus on Manufacturing and Production, alongside a Master of Science in Engineering Technology for Strategy and Security (Strategos) from the University of Genoa. His research interests center on integrating Agent-Based Modeling, System Dynamics, and optimization methodologies to analyze complex systems. Currently, his work leverages Virtual Reality (VR) environments integrated with wearable devices and biosensing technologies to advance health forecasting and healthcare simulation."
  - name: "Siddharth Abrol"
    role: "Doctoral student · 2025–present"
    photo: "/images/students/siddharth-abrol.jpg"
    brief: "Siddharth Vipankumar Abrol is a Ph.D. student in Modeling and Simulation at the University of Central Florida in Orlando. He holds a master's degree in industrial and systems engineering from the University of Florida. His academic and research interests focus on applying quantitative and qualitative methods to improve healthcare systems, with particular emphasis on emergency department workforce well-being and scheduling. His work explores how techniques such as simulation, data analytics, and systems thinking can be used to better understand complex operational challenges in emergency departments."
  - name: "Franklin Castillo"
    role: "Doctoral student · 2024–present"
    photo: "/images/students/franklin-castillo.jpg"
    brief: "Franklin Castillo is a PhD student of Modeling and Simulation at the University of Central Florida, where his research applies macroeconomic agent-based models to the detection of trade-based money laundering. He is a data science and computer programming faculty member at St. Petersburg College and an adjunct instructor of economics at Hillsborough College. His professional background spans over a decade in anti-money laundering operations and technology roles for various financial institutions. He holds certifications as a Project Management Professional (PMP) and a Certified Anti-Money Laundering Specialist (CAMS) and is a Marine Corps veteran with nine years of service."
  - name: "Deepa Fernandes Prabhu"
    role: "Doctoral student · 2024–present"
    photo: "/images/students/deepa-fernandes-prabhu.jpg"
    brief: "Deepa Fernandes Prabhu is a Ph.D. candidate in Modeling and Simulation at the University of Central Florida. She holds graduate degrees from Harvard University and MIT and has a multidisciplinary background spanning engineering, data science, healthcare, and artificial intelligence. Her research focuses on trustworthy healthcare AI, particularly data completeness, generative reconstruction, predictive modeling, uncertainty, and human-centered design. Her current dissertation research develops and evaluates a completeness-aware framework for improving the reliability of predictive AI in healthcare."
  - name: "Ishita Pathuri"
    role: "Master's student · 2026–present"
    photo: "/images/students/ishita-pathuri.jpg"
    brief: "Ishita Pathuri is a graduate student in Computer Science at the University of Central Florida and a Research Assistant in the XR BioSim Lab. Her research interests include artificial intelligence, machine learning, augmented reality, computer vision, and healthcare-focused AI applications. She is currently contributing to the SMART project, which explores AI-enhanced adaptive pediatric emergency training using AR, multimodal biosensing, and real-time personalized feedback. Her broader experience includes generative AI, retrieval-augmented generation, deep learning, and medical imaging applications."
  - name: "Vihaan Gangupanthulu"
    role: "High school student · 2026–present"
    photo: "/images/students/vihaan-gangupanthulu.jpg"
    brief: "Vihaan Gangupanthulu is a rising junior at Orlando Science High School with three years of competitive robotics experience through FTC and FRC, where he serves as the mechanical design lead. His team has competed at State championships and qualified for the FRC World Championship. He joined the XR BioSim Lab in the summer of 2026 to deepen his research experience in robotics and simulation and is excited to contribute to the lab's ongoing projects."
committeeStudents:
  - name: "Md Zahidul Islam"
    program: "Doctoral student in Industrial Engineering"
    brief: "Scheduling models for emergency department clinicians that account for fatigue, burnout, and preferences, integrating empirical evidence with optimization and simulation."
  - name: "Jamila Carpentieri"
    program: "Honors Undergraduate Thesis"
    brief: "Examining Differences in Gaze Behaviors between Pediatric Residents with Varying Levels of Clinical Experience During Pediatric Emergency Simulations."
alumniPrimaryAdvisor:
  - name: "Renita Washburn"
    credit: "Ph.D. Dissertation Advisor, Spring 2026"
    thesisTitle: "Dynamics of Household Food Insecurity Vulnerability: Examining Health, Functional Limitations, and Income Stability."
  - name: "Ankit Prasad"
    credit: "M.S. Thesis Advisor, Summer 2024"
    thesisTitle: "Unveiling Neurophysiological Signatures of Interaction in Immersive Worlds: A Multimodal Study."
  - name: "Shweta Prasad"
    credit: "M.S. Thesis Advisor, Spring 2024"
    thesisTitle: "Investigating the Impact of Pandemic Scenarios on Anesthesiology Department Healthcare Worker Availability: An Agent-Based Modeling Approach."
  - name: "Shaghayegh Mashinkarjavan"
    credit: "M.S. Thesis Advisor, Spring 2024"
    thesisTitle: "Hourly Forecasting of Emergency Department Arrivals for Different ESI Levels."
  - name: "Michael Gonzalez, Charlie Haller, Amber Mossembekker, Jonah Shuda, Madeline Umscheid"
    credit: "B.S. Senior Design, Spring 2024"
    thesisTitle: "VR Integrated Biofeedback for Improving Health."
alumniCommitteeMember:
  - name: "Annamarie Brosnihan"
    credit: "M.S. Thesis committee member, Spring 2026"
    thesisTitle: "Examining Team Leader Visual Dynamics and Team Performance in Pediatric Emergency Simulations."
  - name: "Shivani Gurunath"
    credit: "M.S. Thesis committee member, Spring 2023"
    thesisTitle: "Managing Warehouse Resource and Transportation Plan Stability Using the Flexible Requirements Profile Technique."
  - name: "Mariana Ramirez, Brett Labo, Daniel Bakos, William Bland"
    credit: "B.S. Senior Design committee member, Summer 2026"
    thesisTitle: "Design and Implementation of a Wearable Haptic Glove for Immersive Virtual Reality (VR) Training Simulation."
alumniCapstone:
  - name: "Sai Shravan"
    year: "Spring 2024"
  - name: "Jamiyah C. Castleberry"
    year: "Spring 2024"
  - name: "Jason R. Easterling"
    year: "Spring 2024"
  - name: "Aptheeri Chakradhar Reddy"
    year: "Spring 2024"
  - name: "Abhishek Gowda Lokesh"
    year: "Spring 2024"
  - name: "Kriti Kumari"
    year: "Spring 2024"
  - name: "Adwait Girish Nehete"
    year: "Spring 2024"
  - name: "Supriya Pandita"
    year: "Spring 2024"
  - name: "Raphel Rajkumar"
    year: "Spring 2024"
  - name: "Geethika Uppara"
    year: "Spring 2024"
  - name: "Pochi Reddy Veerannagari"
    year: "Spring 2024"
  - name: "Silviya Antony"
    year: "Fall 2023"
  - name: "Pradeep Narayan"
    year: "Fall 2023"
  - name: "Avinash Vankudoth"
    year: "Spring 2023"
  - name: "Pooja Gambhire"
    year: "Spring 2023"
equipment:
  - name: "EmotiBit"
    photo: ""
    desc: "Multimodal biosensor — EDA, HRV, temperature"
  - name: "Muse 2"
    photo: ""
    desc: "EEG headband for neurophysiological monitoring"
  - name: "fNIRS system"
    photo: ""
    desc: "Functional near-infrared spectroscopy"
  - name: "XR headsets"
    photo: ""
    desc: "VR/AR platforms for immersive simulation studies"
---
